...
import ChallengeDoodle from "@/components/ChallengeDoodle";
import {
      ...
      {items.map((ch, i) => {
        ...
        return (
          <Link
            ...
            className={`card-ink group relative flex flex-col overflow-hidden rounded-2xl bg-paper p-6 transition-transform duration-200 hover:-translate-y-1 ${tilt}`}
            style={{ background: `${ch.color}0d` }}
          >
            {/* the challenge's subject, ghosted off the corner */}
            <ChallengeDoodle
              id={ch.id}
              className="pointer-events-none absolute -bottom-5 -right-4 h-28 w-28 -rotate-6 opacity-[0.16] transition-transform duration-300 group-hover:rotate-0"
            ...
            <h2
              className="relative mt-3 font-display text-2xl font-semibold text-ink group-hover:underline group-hover:decoration-2 group-hover:underline-offset-4"
              style={{ textDecorationColor: ch.color }}
            >
              {ch.title}
            </h2>
            <p className="relative mt-2 flex-1 text-sm leading-6 text-stone">{ch.tagline}</p>
