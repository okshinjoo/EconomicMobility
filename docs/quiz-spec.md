...
**The knowledge check is optional — and short (July 2026 owner pass: it felt too long, and skipping wasn't obvious).** Only the FIRST `MAX_KC_TOPICS` (2) selected topics get quizzed, `KC_QUESTIONS_PER_TOPIC` (2) questions each — four questions max no matter how many topics were picked. Results score only the quizzed topics (`quizzedTopicIds` prop; a footnote explains the spot-check when topics were left out — Where-to-Start still covers every pick). Every check screen carries an amber **"Optional"** banner above the question ("nothing is graded, and you can skip it") and the progress label reads "Knowledge Check (optional)". A footer under the nav offers two opt-outs: **"Skip this question"** (text link — advances without answering; that question simply scores as not-correct) and **"Skip to my results"** (now an outlined BUTTON, not a text link — sets a `skippedKc` flag and jumps straight to results). When `skippedKc` is set, the results page suppresses the per-topic score feedback entirely (no misleading 0/2 scores) and shows a "Based on what you told us" note instead — recommendations are still driven by Q1–Q5. `skippedKc` is persisted in the saved snapshot so a resumed result matches.
