...
**Session 2 — students homepage 10 → 6 bands (~half day).** ✅ **DONE July 14, 2026** — `/students` is **6 bands / 48 visible links** (was 10 / 76): hero · StageDash · **five** pillar posters (Opportunities joined the grid with a New chip; the Deadlines poster carries the first-two-dates line, and `#calendar` lands on the poster grid) · starter guides + a 2-door stay-on-track column (journey + course — the page's only course/journey doors; the tracker door deduped to its poster) · community · one compact "Everything else" band (`#shelf`: college shelf + all-topics / All tools / transfer guide + ASSIST.org, as one-line doors). `/students/deadlines` body links on the homepage: **14 → 2**, as prescribed. ASSIST re-homed to the cc StageDash pool (`lib/studentRecs.ts`, cc now 8 doors, daily rotation). Leak crawl: 0 leaks / 0 broken over 308 pages; anchors `#calendar`/`#shelf` both resolve (Guides ▾ "Student life essentials" still lands on `#shelf`).
...
**Session 3 — one-cross-sell rule on the main frame (~half day+).** ✅ **DONE July 14, 2026** — homepage `/ask` **body doors 5 → 1** (one line in the community band; Community ▾ + footer stay the canonical doors — the Asked-and-answered band was cut, the letters live on `/ask`); the two pointer lines under the topic grid merged to one (both links kept). `ArticleFollowUps` gained `FirstUndonePrompt`: the three prompt cards show **one at a time** (topic quiz → tool → roadmap, first undone; ≤ 2 prompt cards with the site-quiz band — verified on college + home-ownership articles). Topic hubs **7 → 5 bands** (tool card + quiz banner merged into one practice band, the "Keep going" related-topics grid cut — `/learn` owns the topic index, the back link stays; the 🎓 invite chip dropped from the `/learn/college` **hub** via `getStudentPagePaths()`, guide/tool pages keep it). `/students/deadlines` 4-tools band → one All-tools line (41 → 38 links). `/tools` letters promo demoted to a plain registry card (templates promo is the page's one cross-sell; the quiz band was already gone). `/journey/[goal]` verified already compliant (the plan promo renders only on the `/journey` hub). **Re-census (1440×900, July 14):** `/` 9 bands / 62 visible links (the audit's 9 minus the Ask band plus the owner-directed July-14 plan-beta strip; the ≤ 55 estimate is bounded by owner-approved bands — hero, plan strip, questions, numbers, For Students, mission, topics, math, community are all keeps), topic hubs 5 bands / 62–64 links, `/students` 6 / 48, `/tools` 5 / 31, article pages ≤ 2 prompt cards.
