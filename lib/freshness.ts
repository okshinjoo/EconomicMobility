...
export const freshness: FreshnessEntry[] = [
  {
    ...
    lastVerified: "July 18, 2026. TWO vintages by design: the original 100 carry May 2025 OEWS pay (31 with the verified payLow/payHigh/numJobs range, 69 pending the API-quota backfill). WAVES 1–7 added 318 careers (100 → 418, comprehensive) from the BLS Employment Projections table (2024 wage + 2024–34 growth/openings/education) — their May 2025 pay range backfills from the OEWS API later. annualOpenings (2024–34 avg) was added for all via the EP table. Cards/pages show an em dash where a range is still pending; top-coded medians render as \"$239,200+\" via payLabel(). Wave 7 classified ALL 432 remaining EP codes into buckets (residual/professor-by-subject/machine-op/supervisor/clerk-aide-manager variants) to PROVE the floor, then added the last 16 genuinely-searchable roles. 418 is the proven endpoint — every remaining uncovered SOC is a structural variant/residual or lacks an honest annual wage (actors/musicians/military, deliberately excluded). Do not expand further; re-verify existing entries on the OEWS/EP refresh instead.",
