...
export const siteNotes: SiteNote[] = [
  ...
  {
    date: "July 18, 2026",
    title: "The Career Explorer grows to 269 jobs — and now shows how many openings a year",
    body: "We nearly tripled the list, in three waves. The first added the jobs people actually start in and climb from: nursing assistant, medical assistant, cashier, retail, food service, warehouse, security guard, bus driver, construction laborer, painter, industrial mechanic, and the office and management rungs above them. The second went for breadth: more healthcare specialties (nuclear medicine, MRI, cardiovascular techs, opticians, audiologists), the helping professions (psychologist, counselor, social worker), media and design (photographer, journalist, editor, interior and industrial designers), the sciences and engineering (chemical, biomedical, environmental engineers, forensic tech, economist), and the professions (judge, court reporter, more managers). The third filled in the world of work that keeps everything running: the factory floor (tool and die maker, CNC operator, assembler, quality inspector), the oil field and mine, the rails and the water (locomotive engineer, railroad conductor, ship captain, sailor), the front desk and back office (payroll and billing clerks, executive assistant, shipping clerk), and the trades and services around us (barber, glazier, telecom installer, auto body repairer, pest control, casino dealer, lifeguard). We also added a stat we'd been missing: openings a year — positions that open annually across the country, counting both new jobs and the ones freed up when people retire or move on. It's often a truer read on your odds than growth alone: retail is barely growing, but it opens more than half a million jobs a year. And we're honest about the shrinking ones — data entry and payroll clerks are being automated away, and we say so. The newest additions use the 2024 wage figures while we refresh them, and each one's full earning range fills in as we pull it.",
