...
export const siteNotes: SiteNote[] = [
  ...
  {
    date: "July 18, 2026",
    title: "The Career Explorer grows to 210 jobs — and now shows how many openings a year",
    body: "We more than doubled the list, in two waves. The first added the jobs people actually start in and climb from: nursing assistant, medical assistant, cashier, retail, food service, warehouse, security guard, bus driver, construction laborer, painter, industrial mechanic, and the office and management rungs above them — bank teller, loan officer, project manager, financial manager. The second went for breadth: more healthcare specialties (nuclear medicine, MRI, cardiovascular, and dental techs, opticians, audiologists), the helping professions (psychologist, counselor, healthcare social worker), the media and design fields (photographer, journalist, editor, interior and industrial designers, art director), the sciences and engineering (chemical, biomedical, and environmental engineers, microbiologist, forensic tech, economist), more skilled trades (brickmason, boilermaker, crane and power-plant operator), and the work that feeds and moves the country (butcher, baker, farmer, farmworker, taxi and school-bus drivers). We also added a stat we'd been missing: openings a year — positions that open annually across the country, counting both new jobs and the ones freed up when people retire or move on. It's often a truer read on your odds than growth alone: retail is barely growing, but it opens more than half a million jobs a year. The newest additions use the 2024 wage figures while we refresh them, and each one's full earning range fills in as we pull it. More waves are coming — the goal is every major career.",
