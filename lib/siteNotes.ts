...
export const siteNotes: SiteNote[] = [
  ...
  {
    date: "July 18, 2026",
    title: "The Career Explorer now covers 357 jobs — from the dishwasher to the surgeon",
    body: "We built this out in waves. First the jobs people start in and climb from (nursing assistant, cashier, retail, food service, warehouse, security guard, construction trades) and the rungs above them. Then breadth across healthcare, the helping professions, media, the sciences, and the professions. Then the world of work that keeps everything running — the factory floor, the oil field and mine, the rails and the water, the back office. Then a sweep for the common jobs no list should skip (stocker, housekeeper, dishwasher, busser, host, the principal's and CEO's chairs). And then we double-checked our own work, and found we'd been quietly skipping the careers people dream about most — the ones that employ few people but that everyone explores: surgeon, anesthesiologist, and pediatrician; nurse anesthetist; the AI and UX and animation jobs; wildlife biologist, epidemiologist, physicist; film editor, camera operator, and music director; commercial diver and avionics tech; even professional athlete. Those are in now. We also added a stat we'd been missing — openings a year, counting new jobs plus the ones freed up when people retire — because it's often a truer read on your odds than growth alone. And we stay honest at both ends: we mark the shrinking jobs (data entry, payroll clerks) as shrinking, we show the very highest earners as \"$239,200 or more\" rather than a false-precise number, and we leave out the careers we can't give you an honest wage for — actors, musicians, professional gamers, the military — instead of inventing one. Everything comes from real Bureau of Labor Statistics data.",
