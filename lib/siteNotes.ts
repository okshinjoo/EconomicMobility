...
export const siteNotes: SiteNote[] = [
  ...
  {
    date: "July 18, 2026",
    title: "The Career Explorer now covers 402 jobs — a real map of American work",
    body: "We built this out in waves, then checked our own work twice. First the jobs people start in and climb from (nursing assistant, cashier, retail, food service, warehouse, security guard, construction trades) and the rungs above them. Then breadth across healthcare, the helping professions, media, the sciences, and the professions. Then the world of work that keeps everything running — the factory floor, the oil field and mine, the rails and the water, the back office. Then a sweep for the common jobs no list should skip (stocker, housekeeper, dishwasher, busser). A first re-check caught the careers people dream about but that employ few people: surgeon, nurse anesthetist, the AI and animation jobs, wildlife biologist, film editor, even professional athlete. And a final line-by-line pass through the whole federal occupation list caught the last distinct careers hiding among the near-duplicates — computer programmer (different from a software developer), real estate broker (the step above an agent), ER doctor, radiologist, cardiologist, fashion designer, arborist, locksmith, TSA officer, sports referee, marriage and family therapist, and more. We also added a stat we'd been missing — openings a year, counting new jobs plus the ones freed up when people retire. And we stay honest at both ends: we mark the shrinking jobs as shrinking, we show the very highest earners as \"$239,200 or more\" rather than a false-precise number, and we leave out the careers we can't give an honest wage for — actors, musicians, professional gamers, the military — instead of inventing one. What's left uncovered now is genuinely repetitive (a dozen kinds of machine operator, a professor for every subject), not real gaps. Everything comes from real Bureau of Labor Statistics data.",
