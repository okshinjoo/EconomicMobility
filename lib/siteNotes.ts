...
export const siteNotes: SiteNote[] = [
  ...
  {
    date: "July 18, 2026",
    title: "The Career Explorer now covers 323 jobs — a real look at almost any career",
    body: "We more than tripled the list, in four waves, to something close to complete: nearly every career an American might actually consider is now here. The first waves added the jobs people start in and climb from (nursing assistant, cashier, retail, food service, warehouse, security guard, construction trades) and the rungs above them (bank teller, loan officer, the managers). Then breadth: healthcare specialties (nuclear medicine, MRI, cardiovascular techs, opticians, audiologists), the helping professions (psychologist, counselor, social worker), media and design (photographer, journalist, editor, designers), the sciences and professions (chemical and biomedical engineers, economist, judge, court reporter). Then the world of work that keeps everything running — the factory floor, the oil field and mine, the rails and the water (locomotive engineer, ship captain), the back office, and the trades around us. And a final sweep for the common jobs no list should skip: stocker, housekeeper, dishwasher, busser, restaurant host, tutor, preschool and substitute teachers, the principal's and CEO's chairs. We also added a stat we'd been missing: openings a year — positions that open annually across the country, counting both new jobs and the ones freed up when people retire or move on. It's often a truer read on your odds than growth alone: retail is barely growing, but it opens more than half a million jobs a year. And we stay honest about the shrinking ones — data entry and payroll clerks are being automated away, and we say so. Everything comes from real Bureau of Labor Statistics data; the newest additions use the 2024 wage figures while we refresh them, and each one's full earning range fills in as we pull it.",
