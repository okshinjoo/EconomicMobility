...
// source catalog but are not presented publicly as verified scholarships.
...
>(records: readonly T[]): T[] {
  ...
  const missingPassedIds = [...scholarshipAuditPassedIds].filter(
  ...
  const curated = records
    .filter((record) => scholarshipAuditPassedIds.has(record.id))
    .map((record) => {
  ...
  if (curated.length !== SCHOLARSHIP_AUDIT_PASSED) {
    throw new Error(
      `Expected ${SCHOLARSHIP_AUDIT_PASSED} audited scholarships; received ${curated.length}.`,
    );
  }
