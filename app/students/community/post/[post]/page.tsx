...
// The student-framed post page: identical thread to /community/post/[post]
// (canonical points there), but wrapped in the microsite so reading and
// replying never leaves the student frame.
...
export async function generateMetadata({
  ...
  return {
    title: p ? `${p.title} | Empower Community` : "Post | Empower Community",
    robots: { index: false },
    alternates: { canonical: `/community/post/${post}` },
  };
}

export default async function StudentCommunityPostPage({
  params,
}: {
  params: Promise<{ post: string }>;
}) {
      ...
      <section className="bg-paper">
        <div className="mx-auto grid max-w-6xl gap-8 px-6 py-10 lg:grid-cols-[minmax(0,1fr)_19rem] lg:py-14">
          <div className="min-w-0">
            ...
            >
              ← Back to the students community
            </Link>
            <div className="mt-4">
              ...
              <div className="rounded-2xl border border-sand bg-cream p-5">
                ...
                <div className="mt-2 space-y-2.5">
                  {more.map((p) => (
                    <Link
                      key={p.id}
                      href={`/students/community/post/${p.id}`}
