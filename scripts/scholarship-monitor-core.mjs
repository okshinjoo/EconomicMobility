...
const NON_APPLICANT_DATE_LANGUAGE = /\b(?:clubs? to submit (?:a |their )?winner|delegate selections|district deadline|national deadline for all states|public voting|voting (?:period|window)|winners? will be notified|nominate your favorite (?:media )?teacher|registration request portal|teacher[^.]{0,80}(?:honor|honoree))\b/i;
